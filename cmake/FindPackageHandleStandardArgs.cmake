...
macro(_FPHSA_HANDLE_FAILURE_CONFIG_MODE)
  # <PackageName>_CONFIG is set, but FOUND is false, this means that some other
  # of the REQUIRED_VARS was not found:
  if(${_NAME}_CONFIG)
    ...
    if(${_NAME}_CONSIDERED_CONFIGS)
      ...
      if(${_NAME}_NOT_FOUND_MESSAGE)
        if(FPHSA_REASON_FAILURE_MESSAGE)
          string(PREPEND FPHSA_REASON_FAILURE_MESSAGE
...
function(FIND_PACKAGE_CHECK_VERSION version result)
  cmake_parse_arguments(
    PARSE_ARGV 2 FPCV "HANDLE_VERSION_RANGE;NO_AUTHOR_WARNING_VERSION_RANGE"
    "RESULT_MESSAGE_VARIABLE" "")

  if(FPCV_UNPARSED_ARGUMENTS)
    message(
      FATAL_ERROR
        "find_package_check_version(): ${FPCV_UNPARSED_ARGUMENTS}: unexpected arguments"
    )
  endif()
  if("RESULT_MESSAGE_VARIABLE" IN_LIST FPCV_KEYWORDS_MISSING_VALUES)
    message(
      FATAL_ERROR
        "find_package_check_version(): RESULT_MESSAGE_VARIABLE expects an argument"
    )
  endif()
  ...
  if(NOT FPCV_NO_AUTHOR_WARNING_VERSION_RANGE
     AND ${package}_FIND_VERSION_RANGE
     AND NOT FPCV_HANDLE_VERSION_RANGE)
    message(
      AUTHOR_WARNING
  ...
  if(FPCV_HANDLE_VERSION_RANGE AND ${package}_FIND_VERSION_RANGE)
    if((${package}_FIND_VERSION_RANGE_MIN STREQUAL "INCLUDE"
      ...
      set(version_ok TRUE)
      set(version_msg
          "(found suitable version \"${version}\", required range is \"${${package}_FIND_VERSION_RANGE}\")"
      )
    else()
      set(version_msg
          "Found unsuitable version \"${version}\", required range is \"${${package}_FIND_VERSION_RANGE}\""
      )
    endif()
  elseif(DEFINED ${package}_FIND_VERSION)
    if(${package}_FIND_VERSION_EXACT) # exact version required
      ...
      if(version_dots GREATER ${package}_FIND_VERSION_COUNT)
        ...
        # ${package}_FIND_VERSION_COUNT is at most 4 here. Therefore a simple
        ...
        string(REGEX REPLACE "^(${version_regex})\\..*" "\\1" version_head
                             "${version}")
        if(NOT ${package}_FIND_VERSION VERSION_EQUAL version_head)
          set(version_msg
              "Found unsuitable version \"${version}\", but required is exact version \"${${package}_FIND_VERSION}\""
          )
        else()
          set(version_ok TRUE)
          set(version_msg
      ...
      if(${package}_FIND_VERSION VERSION_GREATER version)
        set(version_msg
            "Found unsuitable version \"${version}\", but required is at least \"${${package}_FIND_VERSION}\""
        )
      else()
        set(version_ok TRUE)
        set(version_msg
            "(found suitable version \"${version}\", minimum required is \"${${package}_FIND_VERSION}\")"
        )
      endif()
    endif()
  else()
  ...
  set(${result}
      ...
      PARENT_SCOPE)
  if(FPCV_RESULT_MESSAGE_VARIABLE)
    set(${FPCV_RESULT_MESSAGE_VARIABLE}
        "${version_msg}"
        PARENT_SCOPE)
  endif()
endfunction()
...
function(FIND_PACKAGE_HANDLE_STANDARD_ARGS _NAME _FIRST_ARG)
  ...
  if(${INDEX} EQUAL -1)
    ...
    # In config-mode, we rely on the variable <PackageName>_CONFIG, which is set
    # by find_package() when it successfully found the config-file, including
    # version checking:
  ...
  if(FPHSA_FOUND_VAR)
    ...
    if(FPHSA_FOUND_VAR STREQUAL _FOUND_VAR_MIXED OR FPHSA_FOUND_VAR STREQUAL
                                                    _FOUND_VAR_UPPER)
  ...
  if(DEFINED ${_NAME}_FIND_VERSION)
    if(DEFINED ${FPHSA_VERSION_VAR})
      ...
      find_package_check_version(
        "${_FOUND_VERSION}" VERSION_OK RESULT_MESSAGE_VARIABLE VERSION_MSG
