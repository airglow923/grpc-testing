...
elseif(CMAKE_CXX_COMPILER_LOADED)
  ...
  message(
    FATAL_ERROR "FindThreads only works if either C or CXX language is enabled")
endif()
...
# Internal helper macro. Do NOT even think about using it outside of this file!
...
macro(_check_pthreads_flag)
  if(NOT Threads_FOUND)
    ...
    if(NOT DEFINED THREADS_HAVE_PTHREAD_ARG)
      ...
      elseif(CMAKE_CXX_COMPILER_LOADED)
        set(_threads_src
            ${CMAKE_BINARY_DIR}${CMAKE_FILES_DIRECTORY}/FindThreads/CheckForPthreads.cxx
        )
        configure_file(${CMAKE_CURRENT_LIST_DIR}/CheckForPthreads.c
                       "${_threads_src}" COPYONLY)
      endif()
      try_compile(
        THREADS_HAVE_PTHREAD_ARG ${CMAKE_BINARY_DIR}
        ${_threads_src}
        CMAKE_FLAGS -DLINK_LIBRARIES:STRING=-pthread
        OUTPUT_VARIABLE _cmake_check_pthreads_output)

      string(APPEND _cmake_find_threads_output
...
if(CMAKE_HAVE_PTHREAD_H)
  #
  # We have pthread.h Let's check for the library now.
  #
  set(CMAKE_HAVE_THREADS_LIBRARY)
  if(NOT THREADS_HAVE_PTHREAD_ARG)
    # Check if pthread functions are in normal C library. We list some pthread
    # functions in PTHREAD_C_CXX_TEST_SOURCE test code. If the pthread functions
    # already exist in C library, we could just use them instead of linking to
    # the additional pthread library.
    if(CMAKE_C_COMPILER_LOADED)
    ...
    if(CMAKE_HAVE_LIBC_PTHREAD)
      ...
      # Check for -pthread first if enabled. This is the recommended way, but
      # not backwards compatible as one must also pass -pthread as compiler flag
      # then.
...
if(CMAKE_USE_PTHREADS_INIT)
  if(CMAKE_SYSTEM_NAME MATCHES "HP-UX")
    # Use libcma if it exists and can be used.  It provides more symbols than
    # the plain pthread library.  CMA threads have actually been deprecated:
    # http://docs.hp.com/en/B3920-90091/ch12s03.html#d0e11395
    # http://docs.hp.com/en/947/d8.html but we need to maintain compatibility
    # here. The CMAKE_HP_PTHREADS setting actually indicates whether CMA threads
    # are available.
