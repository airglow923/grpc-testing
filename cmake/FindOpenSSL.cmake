...
macro(_OpenSSL_test_and_find_dependencies ssl_library crypto_library)
  if((CMAKE_SYSTEM_NAME STREQUAL "Linux")
     AND (("${ssl_library}" MATCHES "\\${CMAKE_STATIC_LIBRARY_SUFFIX}$")
...
if(WIN32)
  ...
  set(_OPENSSL_ROOT_PATHS
      "${_programfiles}/OpenSSL" "${_programfiles}/OpenSSL-${_arch}"
      "C:/OpenSSL/" "C:/OpenSSL-${_arch}/")
  unset(_programfiles)
  unset(_arch)
else()
...
set(_OPENSSL_ROOT_HINTS_AND_PATHS HINTS ${_OPENSSL_ROOT_HINTS} PATHS
                                  ${_OPENSSL_ROOT_PATHS})
...
if(WIN32 AND NOT CYGWIN)
  if(MSVC)
    # /MD and /MDd are the standard values - if someone wants to use others, the
    # libnames have to change here too use also ssl and ssleay32 in debug as
    # fallback for openssl < 0.9.8b enable OPENSSL_MSVC_STATIC_RT to get the
    # libs build /MT (Multithreaded no-DLL) In Visual C++ naming convention each
    # of these four kinds of Windows libraries has it's standard suffix: * MD
    # for dynamic-release * MDd for dynamic-debug * MT for static-release * MTd
    # for static-debug

    # Implementation details: We are using the libraries located in the VC
    # subdir instead of the parent directory even though : libeay32MD.lib is
    # identical to ../libeay32.lib, and ssleay32MD.lib is identical to
    # ../ssleay32.lib enable OPENSSL_USE_STATIC_LIBS to use the static libs
    # located in lib/VC/static
    ...
    find_library(
      LIB_EAY_DEBUG
      NAMES
        # When OpenSSL is built with default options, the static library name is
        # suffixed with "_static". Looking the "libcrypto_static.lib" with a
        # higher priority than "libcrypto.lib" which is the import library of
        # "libcrypto.dll".
        ...
        # When OpenSSL is built with the "-static" option, only the static build
        # is produced, and it is not suffixed with "_static".
    ...
    find_library(
      ...
      NAMES
        # When OpenSSL is built with default options, the static library name is
        # suffixed with "_static". Looking the "libssl_static.lib" with a higher
        # priority than "libssl.lib" which is the import library of
        # "libssl.dll".
...
_openssl_test_and_find_dependencies("${OPENSSL_SSL_LIBRARY}"
                                    "${OPENSSL_CRYPTO_LIBRARY}")
if(_OpenSSL_has_dependencies)
...
if(OPENSSL_INCLUDE_DIR AND EXISTS "${OPENSSL_INCLUDE_DIR}/openssl/opensslv.h")
  file(
    STRINGS "${OPENSSL_INCLUDE_DIR}/openssl/opensslv.h" openssl_version_str
  ...
  if(openssl_version_str)
    ...
    # Since OpenSSL 3.0.0, the new version format is MAJOR.MINOR.PATCH and a new
    # OPENSSL_VERSION_STR macro contains exactly that
    file(
      STRINGS "${OPENSSL_INCLUDE_DIR}/openssl/opensslv.h" OPENSSL_VERSION_STR
      REGEX
    ...
    string(
      REGEX
      REPLACE "^.*OPENSSL_VERSION_STR[\t ]+\"([0-9]+\\.[0-9]+\\.[0-9]+)\".*$"
...
find_package_handle_standard_args(
  ...
  VERSION_VAR OPENSSL_VERSION HANDLE_VERSION_RANGE
  HANDLE_COMPONENTS
  FAIL_MESSAGE
...
if(OPENSSL_FOUND)
  if(NOT TARGET OpenSSL::Crypto
    ...
    set_target_properties(
      OpenSSL::Crypto PROPERTIES INTERFACE_INCLUDE_DIRECTORIES
                                 "${OPENSSL_INCLUDE_DIR}")
  ...
  if(NOT TARGET OpenSSL::SSL
    ...
    if(EXISTS "${OPENSSL_SSL_LIBRARY}")
      set_target_properties(
        OpenSSL::SSL PROPERTIES IMPORTED_LINK_INTERFACE_LANGUAGES "C"
  ...
  if("${OPENSSL_VERSION_MAJOR}.${OPENSSL_VERSION_MAJOR}.${OPENSSL_VERSION_FIX}"
     VERSION_GREATER_EQUAL "0.9.8")
    if(MSVC)
      if(EXISTS "${OPENSSL_INCLUDE_DIR}")
